# GOAP Widget Integration Guide

## 📦 Overview

The GOAP Widget is a standalone, embeddable component that brings Goal-Oriented Action Planning research capabilities to any website. It's fully self-contained with no external dependencies required on the host page.
...
<script>
  window.addEventListener('load', function() {
    if (window.GOAPWidget) {
      window.GOAPWidget.init('my-custom-container');
    }
  });
</script>
```
...
window.addEventListener('DOMContentLoaded', function() {
  if (window.GOAPWidget) {
    window.GOAPWidget.init('goap-widget-container');
  }
});
```
...
```javascript
console.log(window.GOAPWidget.version); // "1.0.0"
```
...
<script>
  window.addEventListener('load', function() {
    window.GOAPWidget.init('widget-1');
    window.GOAPWidget.init('widget-2');
  });
</script>
```
...
export default function GOAPWidget() {
  useEffect(() => {
    // Load widget script
  ...
  return <div id="goap-widget-container" />;
}
```
