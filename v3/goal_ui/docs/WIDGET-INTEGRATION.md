# GOAL Widget Integration Guide

## 📦 Overview

The GOAL Widget is a standalone, embeddable component that brings Goal-Oriented Action Planning research capabilities to any website. It's fully self-contained with no external dependencies required on the host page.
...
<script>
  window.addEventListener('load', function() {
    if (window.GoalWidget) {
      window.GoalWidget.init('my-custom-container');
    }
  });
</script>
```
...
window.addEventListener('DOMContentLoaded', function() {
  if (window.GoalWidget) {
    window.GoalWidget.init('goal-widget-container');
  }
});
```
...
```javascript
console.log(window.GoalWidget.version); // "1.0.0"
```
...
<script>
  window.addEventListener('load', function() {
    window.GoalWidget.init('widget-1');
    window.GoalWidget.init('widget-2');
  });
</script>
```
...
export default function GoalWidget() {
  ...
  return <div id="goal-widget-container" />;
}
```
...
- Issues: [GitHub Issues](https://github.com/0x-wzw/goap-ui/issues)
