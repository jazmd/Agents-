/**
 * SmartRetrieval shim — local fallback for ADR-090 pipeline (#bug16a)
 *
 * The upstream `@claude-flow/memory` package no longer exports `smartSearch`
 * (verified against v3.0.0-alpha.14 in node_modules). To unbreak `npm run build`
 * without losing the smart-retrieval feature, this file re-implements the same
 * pipeline locally. It is a verbatim port of the pre-drift implementation
 * (commit d66358149 — "feat: wire SmartRetrieval (ADR-090) into production
 * memory pipeline") so behavior is preserved.
 *
 * Once `@claude-flow/memory` re-exports `smartSearch` (or publishes a
 * `@claude-flow/memory/smart-retrieval` subpath), callers should switch back
 * and this shim can be deleted.
 *
 * Pipeline phases (unchanged from upstream):
 ...
 *
 * @module v3/cli/memory/smart-search-shim
 */
